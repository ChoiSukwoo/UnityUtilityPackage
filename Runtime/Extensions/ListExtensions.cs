...
namespace Suk.Extensions
{
	...
	{
		// Map 메서드
		public static List<TResult> Map<TSource, TResult>(this List<TSource> list, Func<TSource, TResult> action)
		{
			List<TSource> copyList = new List<TSource>(list);
			return copyList.Select(action).ToList();
		}

		public static List<TResult> Map<TSource, TResult>(this List<TSource> list, Func<TSource, int, TResult> action)
		{
			List<TSource> copyList = new List<TSource>(list);
			return copyList.Select(action).ToList();
		}

		public static void Map<TSource>(this List<TSource> list, Action<TSource> action)
		{
			List<TSource> copyList = new List<TSource>(list);
