...
public static class ListExtensions {
   ...
   public static List<TResult> Map<TSource, TResult>(this List<TSource> list, Func<TSource, int, TResult> selector) {
      return list.Select(selector).ToList();
   }
   ...
   public static List<TSource> Filter<TSource>(this List<TSource> list, Func<TSource, bool> predicate) {
      return list.Where(predicate).ToList();
   }

   public static List<TSource> Filter<TSource>(this List<TSource> list, Func<TSource, int, bool> predicate) {
      return list.Where(predicate).ToList();
   }

   // Reduce function: 리스트의 모든 요소를 하나의 값으로 축소합니다.
   public static TResult Reduce<TSource, TResult>(this List<TSource> list, TResult seed, Func<TResult, TSource, TResult> accumulator) {
      return list.Aggregate(seed, accumulator);
   }
