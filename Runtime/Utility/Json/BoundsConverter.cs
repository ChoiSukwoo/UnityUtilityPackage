...
// Bounds 커스텀 컨버터
	public class BoundsConverter : JsonConverter<Bounds>
	{
		...
		public override void WriteJson(JsonWriter writer, Bounds value, JsonSerializer serializer)
		{
			...
			writer.WriteValue(Mathf.RoundToInt(value.center.x * 100f) / 100f);
			writer.WritePropertyName("y");
			writer.WriteValue(Mathf.RoundToInt(value.center.y * 100f) / 100f);
			writer.WritePropertyName("z");
			writer.WriteValue(Mathf.RoundToInt(value.center.z * 100f) / 100f);
			writer.WriteEndObject();

			writer.WritePropertyName("size");
			writer.WriteStartObject();
			writer.WritePropertyName("x");
			writer.WriteValue(Mathf.RoundToInt(value.size.x * 100f) / 100f);
			writer.WritePropertyName("y");
			writer.WriteValue(Mathf.RoundToInt(value.size.y * 100f) / 100f);
			writer.WritePropertyName("z");
			writer.WriteValue(Mathf.RoundToInt(value.size.z * 100f) / 100f);
			writer.WriteEndObject();
