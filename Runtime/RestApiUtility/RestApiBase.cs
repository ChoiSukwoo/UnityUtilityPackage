...
namespace Suk.RestApi {
	internal static class RestApiBase {
		...
		public static async UniTask<T> Get<T>(string url, UnityAction<float> onProgress = null, Dictionary<string, string> headers = null, ContentTypeState expectedType = ContentTypeState.Unknown, AudioType audioType = AudioType.UNKNOWN, CancellationToken cancellationToken = default) {
			using(UnityWebRequest request = UnityWebRequest.Get(url)) {
				// 1. 헤더 설정
				SetRequestHeaders(request, headers);
		...
		public static IEnumerator Post<T>(string url, byte[] bodyData, UnityAction<ApiResponse<T>> onComplete, UnityAction<float> onProgress = null, Dictionary<string, string> headers = null, ContentTypeState expectedType = ContentTypeState.Unknown, AudioType audioType = AudioType.UNKNOWN) {
